...
class MathUtils {
    ...
    static Value eval(Queue<String> infix, HashSet<String> globalNames, HashMap<String, Value> variables) {
        ...
        if (allValuesAreIntegers(rpn, variables)) {
            ...
            return new Value("%" + (LLVMGenerator.reg - 1), Value.VarType.INT, false);
        } else {
            return new Value("%" + (LLVMGenerator.reg - 1), Value.VarType.FLOAT, false);
        }

    }

    private static boolean allValuesAreIntegers (Queue<String> values, HashMap<String, Value> variables) {
        for (String item : values) {
            if (isNumeric(item)) {
                ...
                if (variables.get(item).type != Value.VarType.INT) {
                    return false;
                }
            }
        }
    ...
    private static void handleVariable(String item, Stack<String> stack,
                                       HashMap<String, Value> variables, HashSet<String> globalNames) {
        if (variables.containsKey(item)) {
            Value value = variables.get(item);
